...
def prepare_context(**kwargs):
    ...
    message_user_success = f'Отчет по файлу полетных данных «{file_name}» сформирован!'
    message_user_failure = f'Отчет по файлу полетных данных «{file_name}» не был сформирован!'
    message_logs_success = f'По файлу полетных данных «{path_to_data_file}» сформирован отчет «{path_to_report_file}»! ' + \
    f'В процессе построения были использованы: 1. Файл параметров обработки «{path_to_processing_params}», 2. Файл полетных параметров «{path_to_flight_params}», ' + \
    ...
    f'5. Файл шаблона отчета «{path_to_flight_report_template}».'
    message_logs_failure = f'При формировании отчета по файлу полетных данных «{path_to_data_file}» произошла ошибка! ' + \
    f'В процессе построения были использованы: 1. Файл параметров обработки «{path_to_processing_params}», 2. Файл полетных параметров «{path_to_flight_params}», ' + \
    f'3. Файл словаря наименований признаков «{path_to_feature_names_dict}», 4. Файл словаря наименований диапазонов полета «{path_to_frange_names_dict}», ' + \
    f'5. Файл шаблона отчета «{path_to_flight_report_template}».'

    send_to_user_result = os.path.join(USER_INTERACTION_DIRECTORY, 'Отчет_' + file_name + '.docx')
