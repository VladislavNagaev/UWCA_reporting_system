...
def prepare_context(**kwargs):
    ...
    source_glob = USER_INTERACTION_DIRECTORY + '/*.yaml'
    ...
    if len(path_to_files) > 0:
        ...
        path_to_result_file = os.path.join(DATA_UNVERIFIED_YAML_DIRECTORY, file_name_renamed + '.' + file_format)

        if file_name != file_name_renamed:
            message_user_success = f'Файл «{file}» переименован в «{file_name_renamed}.{file_format}» и сохранен!'
            message_logs_success = f'Файл «{file}» обнаружен в директории «{USER_INTERACTION_DIRECTORY}», переименован в «{file_name_renamed}.{file_format}» и перемещен в директорию «{DATA_UNVERIFIED_YAML_DIRECTORY}»!'
        else:
            message_user_success = f'Файл «{file}» сохранен!'
            message_logs_success = f'Файл «{file}» обнаружен в директории «{USER_INTERACTION_DIRECTORY}» и перемещен в директорию «{DATA_UNVERIFIED_YAML_DIRECTORY}»!'
    ...
    ti.xcom_push(key='RESULT_DIRECTORY', value=DATA_UNVERIFIED_YAML_DIRECTORY)


with DAG(
    dag_id="YandexCloud-YAML_to_LocalStorage",
    ...
    trigger_dag_task = TriggerDagRunOperator(
        task_id=f'trigger-Check_Validity-FlightParams',
        trigger_dag_id='Check_Validity-FlightParams',
        conf={
    ...
    prepare_result_directory_task >> replace_file_task >> [kafka_message_task_1, kafka_message_task_2, trigger_dag_task]
