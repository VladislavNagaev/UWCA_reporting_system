...
def prepare_context(**kwargs):
    ...
    source_glob = USER_INTERACTION_DIRECTORY + '/*.csv'
    ...
    if len(path_to_files) > 0:
        ...
        path_to_result_file = os.path.join(DATA_UNVERIFIED_CSV_DIRECTORY, file_name_renamed + '.' + file_format)

        if file_name != file_name_renamed:
            message_user_success = f'Файл «{file}» переименован в «{file_name_renamed}.{file_format}» и сохранен!'
            message_logs_success = f'Файл «{file}» обнаружен в директории «{USER_INTERACTION_DIRECTORY}», переименован в «{file_name_renamed}.{file_format}» и перемещен в директорию «{DATA_UNVERIFIED_CSV_DIRECTORY}»!'
        else:
            message_user_success = f'Файл «{file}» сохранен!'
            message_logs_success = f'Файл «{file}» обнаружен в директории «{USER_INTERACTION_DIRECTORY}» и перемещен в директорию «{DATA_UNVERIFIED_CSV_DIRECTORY}»!'
    ...
    ti.xcom_push(key='RESULT_DIRECTORY', value=DATA_UNVERIFIED_CSV_DIRECTORY)



with DAG(
    dag_id="YandexCloud-CSV_to_LocalStorage",
    ...
    trigger_dag_task = TriggerDagRunOperator(
        task_id=f'trigger-Check_Validity-FlightData',
        trigger_dag_id='Check_Validity-FlightData',
        conf={
