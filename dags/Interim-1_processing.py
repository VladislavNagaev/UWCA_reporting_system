...
from UWCA_lib import interim_processing_1, get_path_to_file_list, create_control_file
...
def prepare_context(**kwargs):
    ...
    if path_to_file is None:
        path_to_file_list = get_path_to_file_list(
            source_glob=DATA_INTERIM_0_DIRECTORY + '/*.parquet',
            result_glob=DATA_INTERIM_1_DIRECTORY + '/*.parquet',
        )
    ...
    path_to_result_file = os.path.join(DATA_INTERIM_1_DIRECTORY, file_name + '.parquet')
    path_to_processing_params = os.path.join(DATA_EXTERNAL_DIRECTORY, 'processing_params_1' + '.yaml')
    ...
    path_to_control_file = os.path.join(CONTROL_DIRECTORY, 'interim_1' + '.pdf')

    message_user_success = f''
    message_user_failure = f'Обработка файла «{file_name}» прервана на этапе «Interim-1»!'
    message_logs_success = f'Файл полетных данных «{path_to_file}» прошел обработку на этапе «Interim-1». Сформирован актуальный файл полетных данных «{path_to_result_file}»! ' + \
    f'В процессе обработки был использован файл параметров обработки «{path_to_processing_params}»!'
    message_logs_failure = f'При обработке файла полетных данных «{path_to_file}» на этапе «Interim-1» произошла ошибка!.' + \
    f'В процессе обработки был использован файл параметров обработки «{path_to_processing_params}»!'
    ...
    ti.xcom_push(key='SOURCE_DIRECTORY', value=DATA_INTERIM_0_DIRECTORY)
    ti.xcom_push(key='RESULT_DIRECTORY', value=DATA_INTERIM_1_DIRECTORY)
...
with DAG(
    ...
    perform_interim_processing_task = PythonOperator(
        ...
        python_callable=interim_processing_1,
        provide_context=True,
        op_kwargs={
            'path_to_source_file': '{{ task_instance.xcom_pull(key="path_to_source_file") }}',
            'path_to_result_file': '{{ task_instance.xcom_pull(key="path_to_result_file") }}',
    ...
    trigger_dag_task = TriggerDagRunOperator(
        task_id=f'trigger-Interim-2_processing',
        trigger_dag_id='Interim-2_processing',
        conf={
