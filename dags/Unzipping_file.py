...
def prepare_context(**kwargs):
    ...
    path_to_archive = path_to_file

    message_user_success = f''
    message_user_failure = f''
    message_logs_success = f'Файл «{path_to_file}» разархивирован в директорию «{file_base_path}»!'
...
with DAG(
    ...
    perform_unzipping_file_task = BashOperator(
        task_id='perform_unzipping_file',
        bash_command='tar --extract --verbose --directory="$RESULT_DIRECTORY" --file="$path_to_archive"',
        env={
    ...
    prepare_context_task >> prepare_result_directory_task >> perform_unzipping_file_task >> kafka_message_task